...
const std::string memcachedProtocol::noreply_ = "noreply";
...
bool memcachedProtocol::isStorageCommand(const std::string& cmd) const {
  return stroage_cmd_.find(cmd) != stroage_cmd_.end();
}

bool memcachedProtocol::isRetrievalCommand(const std::string& cmd) const {
  return retriveal_cmd_.find(cmd) != retriveal_cmd_.end();
}

bool memcachedProtocol::isOtherCommand(const std::string& cmd) const {
  return other_cmd_.find(cmd) != other_cmd_.end();
}

bool memcachedProtocol::isDeleteCommand(const std::string& cmd) const {
  return delete_cmd_.find(cmd) != delete_cmd_.end();
}

bool memcachedProtocol::isIncrDecrCommand(const std::string& cmd) const {
  return incr_decr_cmd_.find(cmd) != incr_decr_cmd_.end();
}

const std::string& memcachedProtocol::getNoreply() const {
  return noreply_;
}
