...
#include <Log.h>
#include <fas/utils/utilstring.h>
#include <NetAddress.h>
#include <Socket.h>
#include <Poll.h>
#include <Events.h>
#include <fas/utils/utilfile.h>
...
mcacheTask::mcacheTask(hashSPtr hash) :
...
bool mcacheTask::taskCommunition(fas::TcpConnShreadPtr conn, fas::Buffer *buffer) {
...
bool mcacheTask::handleOtherCmd(fas::TcpConnShreadPtr conn, fas::Buffer *buffer) {
  ...
  if (getState() == TaskState::BEGIN_HANDLE_CMD) {
    ...
    LOGGER_TRACE << "send header succeed!" << fas::Log::CLRF;
    ...
    if (noreply_ != protocol_.getNoreply()) {
      ...
      LOGGER_TRACE << "after read response : " << retString << fas::Log::CLRF;
...
bool mcacheTask::handleIncrDecrCmd(fas::TcpConnShreadPtr conn, fas::Buffer *buffer) {
...
bool mcacheTask::handleDeleteCmd(fas::TcpConnShreadPtr conn, fas::Buffer *buffer) {
//  delete <key> [noreply]\r\n
...
bool mcacheTask::handleRetriveCmd(fas::TcpConnShreadPtr conn, fas::Buffer *buffer) {
  ...
  if (getState() == TaskState::BEGIN_HANDLE_CMD) {
    ...
    std::map<fas::Socket_t, std::string> gets_socks_v;
    fas::Poll poller;
    ...
    for (auto itk = keys_.begin(); itk != keys_.end(); ++itk) {
      ...
      LOGGER_TRACE << "ip : " << serverTemp->getIp() << " port : " \
                   << serverTemp->getPort() << fas::Log::CLRF;
...
bool mcacheTask::handleStrogeCmd(fas::TcpConnShreadPtr conn, fas::Buffer *buffer) {

  if (getState() == TaskState::BEGIN_HANDLE_CMD) {
    //cas 命令最多７个参数
    if ((headerItems_.size() != 5) && (headerItems_.size() != 6) && (headerItems_.size() != 7)) {
      LOGGER_ERROR << " head args size : " << headerItems_.size() << fas::Log::CLRF;
      conn->sendString("ERROR\r\n");
      renew();
      return false;
    }
  ...
  if (getState() == TaskState::GET_ALL_DATA) {
    ...
    LOGGER_TRACE << "after write head : " << head_ << fas::Log::CLRF;
    ...
    LOGGER_TRACE << "after write body : " << buffer->retrieveAllAsString() << fas::Log::CLRF;
...
mcacheTask::TaskState mcacheTask::getState() const {
  return this->taskState_;
}

void mcacheTask::setState(TaskState state) {
  this->taskState_ = state;
}

int mcacheTask::getHasReadBytes() const {
  return hasReadBytes_;
}
void mcacheTask::add2HasReadBytes(int bytes) {
  if (bytes <= 0) {
    return;
  }
  hasReadBytes_ += bytes;
...
std::string mcacheTask::getMethod() const {
  return this->method_;
}

std::string mcacheTask::getKey() const {
  return this->key_;
}

std::string mcacheTask::getFlag() const {
  return this->flag_;
}

std::string mcacheTask::getExptime() const {
  return this->exptime_;
}
int mcacheTask::getBytes() const {
  return this->bytes_;
}
